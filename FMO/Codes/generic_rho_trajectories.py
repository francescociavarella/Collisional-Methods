...
# ---------------------------------------------------
    ...
    t_traj = time.time() - t0
    print(f"Elapsed time: {t_traj:.2f} s")
    ...
    np.savez_compressed(
        ...
        # --- Deterministic collisional (trace-ancilla) ---
        rho_trace_coll_exc=rho_trace_coll_exc,
        rho_trace_coll_site=rho_trace_coll_site,
        # --- Monte Carlo: full trajectories (state vectors, exciton basis) ---
        psi_traj=psi_traj.astype(np.complex64), # (N_site, n_times, n_traj)
        jump_counts=jump_counts,                      # (n_times, n_traj)
        total_jumps=total_jumps,                      # (n_times,)
        seeds=seeds,                                  # (n_traj,)
        pop_traj_mean=pop_traj_mean,                         # (n_times, N_site)
        pop_traj_stderr=pop_traj_stderr,                     # (n_times, N_site)
        rho_traj_avg_exc=rho_traj_avg_exc,                # (n_times, N_site, N_site) 
        rho_traj_avg_site=rho_traj_avg_site,              # (n_times, N_site, N_site) 
        # --- Run / model parameters (for reproducibility & analysis) ---
