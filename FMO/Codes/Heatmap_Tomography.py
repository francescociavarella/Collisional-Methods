...
# Estraiamo solo i dati che ci servono per le heatmap
...
# Trasformiamo la singola traiettoria nella base dei siti
...
# Costruiamo la matrice di densità della singola traiettoria: rho(t) = |psi(t)><psi(t)|
...
# ==========================================
# Impostazioni per l'Animazione (Downsampling)
# ==========================================
# Una GIF con 5000 frame peserebbe troppo. Campioniamo circa 150-200 frame totali.
target_frames = 150
stride = max(1, n_times // target_frames)
frames_to_plot = np.arange(0, n_times, stride)

# Colormap divergente: Blu (-1), Bianco (0), Rosso (+1)
cmap = 'RdBu_r'
vmin, vmax = -1.0, 1.0  
...
# ==========================================
# GIF 1: Confronto Redfield vs Average Trajectories (2x2 Grid)
# ==========================================
print("Generating GIF 1: Comparison Redfield vs Average MC...")

fig1, axes1 = plt.subplots(2, 2, figsize=(10, 10))
fig1.suptitle('', fontsize=16, y=0.95)

# Inizializza i 4 plot vuoti
im_red_re = axes1[0, 0].imshow(np.zeros((N_site, N_site)), cmap=cmap, vmin=vmin, vmax=vmax)
im_red_im = axes1[0, 1].imshow(np.zeros((N_site, N_site)), cmap=cmap, vmin=vmin, vmax=vmax)
im_avg_re = axes1[1, 0].imshow(np.zeros((N_site, N_site)), cmap=cmap, vmin=vmin, vmax=vmax)
im_avg_im = axes1[1, 1].imshow(np.zeros((N_site, N_site)), cmap=cmap, vmin=vmin, vmax=vmax)

setup_heatmap_axis(axes1[0, 0], "Redfield: Real Part")
setup_heatmap_axis(axes1[0, 1], "Redfield: Imaginary Part")
setup_heatmap_axis(axes1[1, 0], "Avg MC: Real Part")
setup_heatmap_axis(axes1[1, 1], "Avg MC: Imaginary Part")

fig1.colorbar(im_red_im, ax=axes1.ravel().tolist(), shrink=0.8, pad=0.05, aspect=30)

def update_fig1(frame_idx):
    t = times[frame_idx]
    fig1.suptitle(f'Density Matrix Dynamics (Theta={theta_deg}°)\nTime = {t:.1f} fs', fontsize=16)
    
    im_red_re.set_data(np.real(rho_redfield_site[frame_idx]))
    im_red_im.set_data(np.imag(rho_redfield_site[frame_idx]))
    im_avg_re.set_data(np.real(rho_traj_avg_site[frame_idx]))
    im_avg_im.set_data(np.imag(rho_traj_avg_site[frame_idx]))
    return [im_red_re, im_red_im, im_avg_re, im_avg_im]

ani1 = animation.FuncAnimation(fig1, update_fig1, frames=frames_to_plot, blit=False)
path_gif1 = os.path.join(Output_dir, f"Heatmap_Comparison_Theta_{theta_str}.gif")
ani1.save(path_gif1, writer='pillow', fps=15)
plt.close(fig1)
...
path_gif2 = os.path.join(Output_dir, f"Heatmap_SingleTraj_Theta_{theta_str}.gif")
ani2.save(path_gif2, writer='pillow', fps=15)
plt.close(fig2)
