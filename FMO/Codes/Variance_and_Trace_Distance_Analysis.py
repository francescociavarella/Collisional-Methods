...
if 'rho_redfield_site' in data and 'rho_traj_avg_site' in data:
    ...
    print("Warning: Density matrices not found. Trace distance plots will fail.")
...
# for N_sub in N_list:
#     psi_sub = psi_traj_site[:, :, :N_sub]
#     rho_sub_avg = np.einsum('itk, jtk -> tij', psi_sub, np.conjugate(psi_sub)) / N_sub
    
#     td_time_sub = np.zeros(n_times)
#     for t in range(n_times):
#         td_time_sub[t] = trace_distance_generic_njit(rho_redfield_site[t], rho_sub_avg[t])
    
#     mean_td_values.append(np.mean(td_time_sub))
#     max_td_values.append(np.max(td_time_sub))
...
# ==========================================
# PLOT 7: HEATMAP DELLA DENSITÀ (UN GRAFICO PER OGNI SITO)
# ==========================================
