...
\begin{document}
	...
	\section{Fundamental Concepts of Stochastic Calculus}
	...
	\begin{equation}
		\mathbb{E}[dW] = 0, \quad \mathbb{E}[dW^2] = dt
	\end{equation}
	...
	\subsection{It\^o Algebra}
	
	Because $dW$ scales as $\sqrt{dt}$, second-order terms cannot be neglected when performing differential expansions, unlike in classical calculus where $dt^2 \to 0$. In It\^o calculus, the following strict algebraic rules apply when manipulating stochastic differentials:
	\begin{align}
		dW^2 &= dt \\
		dW dt &= 0 \\
		dt^2 &= 0
	\end{align}
	The relation $dW^2 = dt$ is the cornerstone of It\^o calculus: the square of the noise increment becomes completely deterministic.
	...
	For discontinuous measurement records, such as photon counting or quantum jumps, the noise is initially modeled using a macroscopic discrete Poisson process. Let $N(t)$ be the total number of events up to time $t$. Over a finite time interval $\Delta t$, assuming a mean event rate $\lambda$, the probability of observing exactly $k$ events is governed by the Poisson distribution:
	\begin{equation}
		P(N(t)=k) = \frac{(\lambda \Delta t)^k e^{-\lambda \Delta t}}{k!}
	\end{equation}
	...
	\begin{align}
		\mathbb{E}[N(t)] = \sum_{k=0}^{\infty} k P(k) = \sum_{k=0}^{\infty} k  \frac{(\lambda t)^k e^{-\lambda t}}{k!} = \lambda \Delta t \\
		\text{Var}(N(t)) = \mathbb{E}[N(t)^2] - (\mathbb{E}[N(t)])^2 = \lambda \Delta 
	\end{align}
	
	In the context of stochastic differential equations, we focus on the infinitesimal increment $dn = N(t+dt) - N(t)$, which represents the number of jumps occurring in the microscopic time step $dt$. To understand the statistical behavior of $dn$, we take the limit $\Delta t \to dt$ and expand the exponential term to the first order, since terms of order $dt^2$ are negligible in It\^o calculus:
	\begin{equation}
		e^{-\lambda dt} = 1 - \lambda dt + \mathcal{O}(dt^2) \approx 1 - \lambda dt
	\end{equation}
	...
	\begin{align}
		P(0) &= \frac{(\lambda dt)^0 e^{-\lambda dt}}{0!} \approx 1 - \lambda dt \\
		P(1) &= \frac{(\lambda dt)^1 e^{-\lambda dt}}{1!} = \lambda dt(1 - \lambda dt) \approx \lambda dt \\
		P(k \ge 2) &\propto \mathcal{O}(dt^2) \approx 0
	\end{align}
	...
	\begin{equation}
		\mathbb{E}[dn] = 0 \cdot P(0) + 1 \cdot P(1) = \lambda dt
	\end{equation}
	...
	\begin{equation}
		\text{Var}(dn) = \lambda dt - (\lambda dt)^2 = \lambda dt
	\end{equation}
	...
	\begin{equation}
		\text{Var}(dn) = P_{0} \, P_{1} = \lambda dt \left(1 - \lambda dt \right) = \lambda dt - (\lambda dt)^2 = \lambda dt
	\end{equation}
	...
	\begin{equation}
		d\ket{\psi(t)} = -\frac{i}{\hbar} H \ket{\psi(t)} dt - \frac{1}{2} \Gamma \left( c^\dagger c - \langle c^\dagger c \rangle \right) \ket{\psi(t)} dt + \left( \frac{c}{\sqrt{\langle c^\dagger c \rangle}} - \mathbb{I} \right) \ket{\psi(t)} dn
		\label{eq:generic_QJ}
	\end{equation}
	...
	\subsection{Quantum State Diffusion (SD) Unraveling}
	...
	\subsubsection*{Homodyne Detection (Real Noise Unraveling)}
	
	If the measurement apparatus extracts a single continuous stream of real classical information (homodyne detection), the system is driven by a real Wiener increment $dW$, which satisfies the It\^o relation $dW^2 = dt$. The explicitly norm-preserving, non-linear equation requires compensating only for the Hermitian part of the jump operator:
	\begin{equation}
		d\ket{\psi(t)} = -\frac{i}{\hbar}H\ket{\psi(t)}dt - \frac{1}{2}\Gamma\left(c^\dagger c - \langle c + c^\dagger \rangle c + \frac{1}{4}\langle c + c^\dagger \rangle^2 \right)\ket{\psi(t)}dt + \sqrt{\Gamma}\left(c - \frac{\langle c + c^\dagger \rangle}{2}\right)\ket{\psi(t)}dW
		\label{eq:SD_homodyne}
	\end{equation}
	...
	\subsubsection*{Heterodyne Detection (Complex Noise Unraveling)}
	
	If the measurement extracts information on two conjugate quadratures simultaneously (heterodyne detection, corresponding to the original Gisin-Percival QSD formulation), the unraveling is driven by a complex Wiener increment $d\xi = (dW_1 + i dW_2)/\sqrt{2}$. This complex noise satisfies $|d\xi|^2 = dt$ and $d\xi^2 = 0$. In this case, preserving the norm requires balancing the full complex expectation value $\langle c \rangle$:
	\begin{equation}
		d\ket{\psi(t)} = -\frac{i}{\hbar} H \ket{\psi(t)} dt - \frac{1}{2} \Gamma \left( c^\dagger c - 2\langle c^\dagger \rangle c + \langle c^\dagger \rangle \langle c \rangle \right) \ket{\psi(t)} dt + \sqrt{\Gamma} (c - \langle c \rangle) \ket{\psi(t)} d\xi
		\label{eq:SD_heterodyne}
	\end{equation}
	...
	\subsection{Linear (Unnormalized) Stochastic Equations}
	
	In analytical calculations and numerical simulations, it is often much more efficient to use the linear (or homogeneous) versions of the SSEs. In this approach, we drop the non-linear expectation value terms, leading to the evolution of an unnormalized state vector $\ket{\tilde{\psi}(t)}$. The physical state is then recovered by manually normalizing the vector: $\ket{\psi(t)} = \ket{\tilde{\psi}(t)} / \sqrt{\braket{\tilde{\psi}(t)|\tilde{\psi}(t)}}$.
	
	To write these equations compactly, we introduce the non-Hermitian effective Hamiltonian:
	\begin{equation}
		H_{\text{eff}} = H - i\frac{\hbar}{2} \Gamma c^\dagger c
	\end{equation}
	
	For the \textbf{Quantum Jump} unraveling, the linear SSE simplifies drastically. The state evolves under the effective Hamiltonian until a jump occurs, without any continuous norm-preserving feedback:
	\begin{equation}
		d\ket{\tilde{\psi}(t)} = \left[ -\frac{i}{\hbar} H_{\text{eff}} dt + \left( c - \mathbb{I} \right) dn \right] \ket{\tilde{\psi}(t)}
	\end{equation}
	
	Similarly, for the \textbf{Quantum State Diffusion} driven by real noise $d\xi$, the removal of the continuous feedback terms $\langle c \rangle$ and $\langle c^\dagger c \rangle$ yields a purely linear stochastic differential equation:
	\begin{equation}
		d\ket{\tilde{\psi}(t)} = \left[ -\frac{i}{\hbar} H_{\text{eff}} dt + \sqrt{\Gamma} c dW \right] \ket{\tilde{\psi}(t)}
	\end{equation}
