...
Substituting Eqs.~\eqref{eq:cos_cdt_expansion} and \eqref{eq:sin_cdt_expansion} into 
...
Similarly, substituting Eqs.~\eqref{eq:cos2_cdt_expansion} and \eqref{eq:sin2_cdt_expansion} 
...
(cf.\ Eq.~\eqref{eq:standard_SD_SSE}).

Substituting the expansions of $\mathcal{M}_{0,\chi_{+}^{a}}$ 
(Eq.~\eqref{eq:M0_chiplus_expanded}) and $1/\sqrt{P_{\chi_{+}^{a}}}$ 
(Eq.~\eqref{eq:expansion_Pchiplus}), and retaining all terms up to $O(\Delta t)$ 
...
Substituting Eqs.~\eqref{eq:norm_Mchiplus} and \eqref{eq:norm_Mchiminus} into the 
...
\end{equation}
By defining the raw homodyne measurement record as $ d\tilde{Y} = (2dn - 1)\sqrt{\Delta t} $, setting $ dt = \Delta t $, and substituting the definitions of $ c $ and $ \langle c + c^\dagger \rangle $ into Eq.~\eqref{eq:raw_SD_update}, we can explicitly identify the raw drift operator $ A_{raw} $ and the diffusion operator $ B $:
\begin{align}
