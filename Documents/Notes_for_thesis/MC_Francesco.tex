...
.
	  ...
	  \begin{gather}
	  	\mathcal{F}_{0} = \left| \braket{\Psi_{S}^{PRE} | \Psi_{S}^{POST}}_{\mathcal{M}_{0}} \right|^2 \\
	  	\mathcal{F}_{1} = \left| \braket{\Psi_{S}^{PRE} | \Psi_{S}^{POST}}_{\mathcal{M}_{1}} \right|^2 
	  \end{gather}
	  ...
	  \end{align}
	  It's important to point out that the value of $ c = \cos^{2} \left( c_{j} \Delta t \right) \approx 1 $ due to the chosen parameters $ c_j $ and $ dt $. This gives consequentially $ 1 - c = \sin^{2} \left( c_{j} \Delta t \right) \approx 10^{-7} \approx 0 $, which is the coefficient that weights $ \left| \braket{\sigma_{z}} \right|^2 $, that could essentially be considered irrelevant.
	  ...
	  First, since $ \left| \braket{\sigma_{z}} \right|^2 \in [0,1] $ it's possible to plot the value of Eq.\ref{eq:fidelity_M0} and Eq.\ref{eq:fidelity_M1} as a function of $ g_{0} $ and $ \left| \braket{\sigma_{z}}^{j} \right| $
	  
	  Da questo si vede che la curvatura della parabola è molto piccola in quanto il termine quadratico in sigma z è pesato dal sin cjdt che è circa $10^{-10}$, quindi di fatto trascurabile
	  
	  it's important to calculate quanto spesso uno stato e quindi un valore del valore di aspettazione di sigma z viene campionato con diversi unravleing scheme (cosa degli istogrammi).
	  
	  valutare con diversi valori di aspettazione di sigma z come cambia l'andamento delle quantià che determinano l'intensità della variazione con diversi ssitemi di unraveling. Qui si può fare un plot 3D o diversi plot selezionando o metodi diversi di unraveling o diversi valori di aspettazione.
	  
	  altro plot da fare e quanto gli autostati o in generale i diversi possibili valori dei componenti della sferaa di bloch vengono "campionati durante la dinamica"; questo perchè l'intensità di transiiozne dipende dallo stato in cui si trova il sistema, ad esmepio se sono in autofunzione di sigma z ho cambiamento nullo anche in qj limit. misura su diverse dinamiche con stesso schema di unraveling, misura media su diverse traiettorie; misuro i valori di aspettazione di sigma z ?
	  ciò che mi aspetto è che un andamento circa uguale per tutti gli shemi di unraveling dove tutti gli stai/valori di aspettazione hanno la stessa probabilità di essere campionati.
	  
	  Invece ciò che vediamo nei graficic seguenti sono delle sorte di lorentziane cetrate in 0 in quanto sto misurando sz otimes idensity e quindi solo sz per uno dei due siti, che mi da la diffeerenza di P10 - P01, mediata su più traiettorie che quidi è la lindbald, che tende a 0.5 per ogni P quindi sono + popolati gli stati la cui diffrenza tende a zero.
	  occhio che Sz su entrambi i siti vuol dire Sz otime 1 - 1 otimes Sz che fa 0!!! quidni il secondo termine che era piccolo èperchè èesato dal seno va effettivamente a zero!!
	  
	  \graphicspath{ {/home/francesco/Collisional_Methods/Results/Plot/Probability/Sx_Sy_Sz} }
	  \begin{figure}[H]
	  	\centering
	  	\includegraphics[width=0.7\textwidth]{Sz_distribution_intermediate.png}
	  	\caption{Distribution of the $ \left| \braket{\sigma_{z}^{j}} \right| $, recorded during the dynamics and average over 10.000 Trajectories}
	  	\label{fig:Sz_avg}
	  \end{figure}
	  ...
	  \begin{figure}[H]
	  	\centering
	  	\includegraphics[width=0.7\textwidth]{Sz_distribution_close_to_90_deg.png}
	  	\caption{Distribution of the $ \left| \braket{\sigma_{z}^{j}} \right| $, recorded during the dynamics and average over 10.000 Trajectories, for $ \theta $ angle close to 90°}
	  ...
	  \begin{figure}[H]
	  	\centering
	  	\includegraphics[width=0.7\textwidth]{Sz_square_distribution_intermediate.png}
	  	\caption{Distribution of the $ \left| \braket{\sigma_{z}^{j}} \right|^2 $, recorded during the dynamics and average over 10.000 Trajectories}
	  ...
	  \begin{figure}[H]
	  	\centering
	  	\includegraphics[width=0.7\textwidth]{Sz_square_distribution_close_to_90_deg.png}
	  	\caption{Distribution of the $ \left| \braket{\sigma_{z}^{j}} \right|^2 $, recorded during the dynamics and average over 10.000 Trajectories, for $ \theta $ angle close to 90°}
