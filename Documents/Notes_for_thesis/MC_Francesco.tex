...
Note that : 
\begin{gather}
	\mathcal{M}_{0}^{j \dagger}\mathcal{M}_{0}^{j}  = P_{0_{a}} \mathbb{I}^{j} \notag \\
	\mathcal{M}_{1}^{j \dagger}\mathcal{M}_{1}^{j}  = P_{1_{a}} \mathbb{I}^{j} \notag
\end{gather}
...
\begin{align}
	nome da inventare = 1 - \left| \braket{\Psi_{S}^{PRE} | \Psi_{S}^{POST}}_{\mathcal{M}_{0}^{j}} \right|^2 \\
	nome da inventare = 1 - \left| \braket{\Psi_{S}^{PRE} | \Psi_{S}^{POST}}_{\mathcal{M}_{1}^{j}} \right|^2
\end{align}
...
As seen before the fidelity calculated in Eq.\ref{eq:fidelity_M0} and Eq.\ref{eq:fidelity_M1} are referred to the variation induced by the collision with a single site. If we want to calculate the fidelity referred to the total variation of the system ( $ \ket{\Psi} = \alpha  \ket{\psi_{S}^{1}} + \beta \ket{\psi_{S}^{2}} $ )

To demonstrate rigorously why the global Kraus operator for the outcome $(0,0)$ is the tensor product of the single local operators, namely $\mathcal{M}_{00}^{tot} = \mathcal{M}_0^{(1)} \otimes \mathcal{M}_0^{(2)}$, we must start from the fundamental definition of the physical interaction in our two-site model.

The total system is composed of three distinct entities: the dimer (which lives in its Hilbert space $\mathcal{H}_S = \mathcal{H}_1 \otimes \mathcal{H}_2$), the first ancilla (space $\mathcal{H}_{a_1}$), and the second ancilla (space $\mathcal{H}_{a_2}$).

The total collisional Hamiltonian is the sum of the independent interactions on the two sites:
\begin{equation}
	\mathcal{H}_{Coll}^{tot} = \mathcal{H}_{Coll}^{(1)} + \mathcal{H}_{Coll}^{(2)}
\end{equation}

Since the interaction $\mathcal{H}_{Coll}^{(1)}$ acts only on site 1 and ancilla 1, while $\mathcal{H}_{Coll}^{(2)}$ acts only on site 2 and ancilla 2, these two operators commute, meaning $[\mathcal{H}_{Coll}^{(1)}, \mathcal{H}_{Coll}^{(2)}] = 0$. This property is crucial because it allows us to factorize the total unitary evolution operator of the entire composite system:
\begin{equation}
	\mathcal{U}_{tot} = \exp[-i (\mathcal{H}_{Coll}^{(1)} + \mathcal{H}_{Coll}^{(2)}) \Delta t] = \exp[-i \mathcal{H}_{Coll}^{(1)} \Delta t] \otimes \exp[-i \mathcal{H}_{Coll}^{(2)} \Delta t] = \mathcal{U}^{(1)} \otimes \mathcal{U}^{(2)}
\end{equation}
Now we apply the formal definition of the Kraus operator. The initial state of the bath (the two ancillas) is a factorized state $|\Phi_B\rangle = |\phi_{a_1}\rangle \otimes |\phi_{a_2}\rangle$.

The global Kraus operator for measuring ancilla 1 in state $|0_{a_1}\rangle$ and ancilla 2 in state $|0_{a_2}\rangle$ is obtained by projecting the total evolution operator $\mathcal{U}_{tot}$ onto the initial and final states of the bath:
\begin{equation}
	\mathcal{M}_{00}^{tot} = (\bra{0_{a_1}} \otimes  \bra{0_{a_2}}) \mathcal{U}_{tot} (\ket{\phi_{a_1}}\otimes \ket{\phi_{a_2}})
\end{equation}

Substituting $\mathcal{U}_{tot} = \mathcal{U}^{(1)} \otimes \mathcal{U}^{(2)}$ into this expression, the terms naturally separate to act on their respective subspaces:
\begin{equation}
	\mathcal{M}_{00}^{tot} = (\langle 0_{a_1}| \mathcal{U}^{(1)} |\phi_{a_1}\rangle) \otimes (\langle 0_{a_2}| \mathcal{U}^{(2)} |\phi_{a_2}\rangle)
\end{equation}

The terms in parentheses are exactly the definitions of our local Kraus operators for the single sites. Specifically, $\langle 0_{a_1}| \mathcal{U}^{(1)} |\phi_{a_1}\rangle = \mathcal{M}_0^{(1)}$ (acting on site 1) and $\langle 0_{a_2}| \mathcal{U}^{(2)} |\phi_{a_2}\rangle = \mathcal{M}_0^{(2)}$ (acting on site 2). 

Consequently, the global map for this specific trajectory step is precisely:
\begin{equation}
	\mathcal{M}_{00}^{tot} = \mathcal{M}_0^{(1)} \otimes \mathcal{M}_0^{(2)}
\end{equation}
Like this we can calculate all the four total operators, defining before :
\begin{gather}
	a_{0} = g_{0} \cos \left(c_{j} \Delta t\right) \quad ; \quad b_{0} = - g_{1} \sin \left(c_{j} \Delta t\right) \notag \\
	a_{1} = g_{1} \cos \left(c_{j} \Delta t\right) \quad ; \quad b_{1} = g_{0} \sin \left(c_{j} \Delta t\right) \notag \\
	\mathcal{M}_{00}^{tot} = a_0^2 \mathbb{I} - i a_0 b_0 (\sigma_z^1 + \sigma_z^2) - b_0^2 \sigma_z^1 \sigma_z^2 \\
	\mathcal{M}_{01}^{tot} = a_0 a_1 \mathbb{I} - i a_1 b_0 \sigma_z^1 - i a_0 b_1 \sigma_z^2 - b_0 b_1 \sigma_z^1 \sigma_z^2 \\
	\mathcal{M}_{10}^{tot} = a_0 a_1 \mathbb{I} - i a_0 b_1 \sigma_z^1 - i a_1 b_0 \sigma_z^2 - b_0 b_1 \sigma_z^1 \sigma_z^2 \\
	\mathcal{M}_{11}^{tot} = a_1^2 \mathbb{I} - i a_1 b_1 (\sigma_z^1 + \sigma_z^2) - b_1^2 \sigma_z^1 \sigma_z^2
\end{gather}
remember that : 
\begin{gather}
	\sigma_{z} = \sigma_{z}^{1} \otimes \mathbb{I} + \mathbb{I} \otimes \sigma_{z}^{2}
...
\begin{align}
	\left| \braket{\sigma_{z}} \right| = P_{01} - P_{10} - \left( P_{01} - P_{10} \right) = 0
\end{align}
and in the specific case of our system, where only the excited state are non zero, i.e. $ \ket{01} $ and $ \ket{10} $ we obtain that : 
\begin{equation}
	\sigma_{z}^{1}\sigma_{z}^{2} = - \mathbb{I}
\end{equation}
so $ M^{tot} $ simplifies as :\begin{gather}
	\mathcal{M}_{00}^{tot} = (a_0^2 + b_0^2) \mathbb{I} \\
	\mathcal{M}_{01}^{tot} = (a_0 a_1 + b_0 b_1) \mathbb{I} - i (a_1 b_0 - a_0 b_1) \sigma_z^1 \\
	\mathcal{M}_{10}^{tot} = (a_0 a_1 + b_0 b_1) \mathbb{I} - i (a_0 b_1 - a_1 b_0) \sigma_z^1 \\
	\mathcal{M}_{11}^{tot} = (a_1^2 + b_1^2) \mathbb{I}
\end{gather}
replacing \textit{a} and \textit{b} coefficient : 
\begin{gather}
	\mathcal{M}_{00}^{tot} = \left[ g_0^2 \cos^2(c_j \Delta t) + g_1^2 \sin^2(c_j \Delta t) \right] \mathbb{I} \\
	\mathcal{M}_{01}^{tot} = g_0 g_1 \left[ \cos^2(c_j \Delta t) - \sin^2(c_j \Delta t) \right] \mathbb{I} + i \sin(c_j \Delta t) \cos(c_j \Delta t) \sigma_z^1 \\
	\mathcal{M}_{10}^{tot} = g_0 g_1 \left[ \cos^2(c_j \Delta t) - \sin^2(c_j \Delta t) \right] \mathbb{I} - i \sin(c_j \Delta t) \cos(c_j \Delta t) \sigma_z^1 \\
	\mathcal{M}_{11}^{tot} = \left[ g_1^2 \cos^2(c_j \Delta t) + g_0^2 \sin^2(c_j \Delta t) \right] \mathbb{I}
\end{gather}
